...
- `docker compose up` — launch Postgres, MinIO, the embedding service, and the Streamlit app; Postgres applies `/docker/postgres/initdb.d/*.sql` on first boot to create the schema and indexes automatically.
...
- `docker compose run --rm app uv run scripts/build_embeddings.py` — fill missing pgvector embeddings (respects `EMBEDDING_BATCH_SIZE`/`EMBEDDING_API_TIMEOUT`). Pass `--reset` to null out existing vectors (after chunk strategy changes) or `--document-id <uuid>` to target specific documents.
- `docker compose run --rm app uv run python scripts/export_feedback_pairs.py --output data/feedback_pairs.jsonl` — convert stored thumbs-up/down into preference pairs.
- `docker compose run --rm app uv run python scripts/finetune_lora_dpo.py --dataset data/feedback_pairs.jsonl` — train/update the LoRA adapter via DPO.
